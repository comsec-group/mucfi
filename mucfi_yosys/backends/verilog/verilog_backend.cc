...
bool dump_cell_expr(std::ostream &f, std::string indent, RTLIL::Cell *cell, const RTLIL::Module *module)
{
...
#undef HANDLE_BINOP
	...
	if (RTLIL::builtin_ff_cell_types().count(cell->type))
	{
		...
		for (int i = 0; i < chunks; i++)
		{
			...
			dump_attributes(f, indent, cell->attributes);
      f << stringf("/* PC_TAINT_INFO MODULE_NAME %s */\n", id(module->name, false).c_str());
      f << stringf("/* PC_TAINT_INFO STATE_NAME %s */\n", reg_bit_name.c_str());
			if (ff.has_clk)
			{
				// FFs.
...
void dump_cell(std::ostream &f, std::string indent, RTLIL::Cell *cell, const RTLIL::Module *module)
{
	...
	if (cell->type[0] == '$' && !noexpr) {
		if (dump_cell_expr(f, indent, cell, module))
			return;
	}
...
void dump_module(std::ostream &f, std::string indent, RTLIL::Module *module)
{
  ...
  for (auto cell : module->cells()) {
		dump_cell(f, indent + "  ", cell, module);
  }
