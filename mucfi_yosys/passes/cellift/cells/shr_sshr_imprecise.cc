...
/**
 * Only propagate taint of the shifted value, treating the shift amount as control input.
 * @param module the current module instance
 * @param cell the current cell instance
 *
 * @return keep_current_cell
...
bool cellift_shr_sshr_data_flow(RTLIL::Module *module, RTLIL::Cell *cell, unsigned int num_taints, std::vector<string> *excluded_signals) {
    ...
    for (unsigned int taint_id = 0; taint_id < num_taints; taint_id++) {
        ...
        // Apply an identical $shr to A's taint input.
        RTLIL::Wire* taint_shr_wire = module->addWire(NEW_ID, output_width);
        RTLIL::Cell* taint_shr_cell;
        if (cell->type == ID($sshr))
            taint_shr_cell = module->addSshr(NEW_ID, port_taints[A][taint_id], ports[B], port_taints[Y][taint_id]);
        else
            taint_shr_cell = module->addShr(NEW_ID, port_taints[A][taint_id], ports[B], port_taints[Y][taint_id]);
    }
