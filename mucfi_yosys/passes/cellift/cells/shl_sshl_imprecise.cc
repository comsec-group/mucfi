...
bool cellift_shl_sshl_data_flow(RTLIL::Module *module, RTLIL::Cell *cell, unsigned int num_taints, std::vector<string> *excluded_signals) {
    ...
    for (unsigned int taint_id = 0; taint_id < num_taints; taint_id++) {
        ...
        // Apply an identical $shl to A's taint input.
        RTLIL::Cell* taint_shl_cell;
        if (cell->type == ID($sshl))
            taint_shl_cell = module->addSshl(NEW_ID, port_taints[A][taint_id], ports[B], port_taints[Y][taint_id]);
        else
            taint_shl_cell = module->addShl(NEW_ID, port_taints[A][taint_id], ports[B], port_taints[Y][taint_id]);
    }
