...
static inline long tb_run_ticks_stoppable(Testbench *tb, int simlen, bool reset = false) {
  ...
  for (; step_id < simlen; step_id++) {
    ...
    if (!taint_already_seen &&
        tb->module_->rootp->vlSymsp->TOP.picorv32_tiny_soc->data_mem_rdata_t0) {
    ...
    if (!got_pc_tainted &&
      tb->module_->rootp->vlSymsp->TOP__picorv32_tiny_soc__i_picorv32_mem_top__i_picorv32.next_pc_t0) {
      std::cout << "PC tainted! Stopping the benchmark after " << RUNMORETICKS_AFTER_STOP << " more ticks." << std::endl;
      std::cout << "Approximative PC at taint time: 0x" << std::hex << tb->module_->rootp->vlSymsp->TOP__picorv32_tiny_soc__i_picorv32_mem_top__i_picorv32.next_pc << std::dec << std::endl;
      got_stop_req = true;
      got_pc_tainted = true;
    }
    ...
    if (!got_mem_tainted_addr &&
      tb->module_->rootp->vlSymsp->TOP.picorv32_tiny_soc->data_mem_addr_t0) {
