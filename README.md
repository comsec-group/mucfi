...
4. Copy an example property instantiation file, e.g. /mucfi/verification/scarv_ift_pregenerated/formal/properties/cellift_scarv_ct_instr_all.sv and adapt if necessary: You need to provide the clock and reset signal and adapt the instruction word sampling. It needs to be sampled in the same clock cycle as the register reading condition is checked. That means, if the condition looks into the past (starts with $past), the instruction word needs to be sampled into a register (see the example in Kronos). If it doesn't, it needs to be sampled with an immediate assignment (see Ibex).
